...
# File containing utility functions used multiple times across different methods and functions

def events_to_vector(events, n_seg):
    """
    returns the copy number profile change corresponding to a list of events
...
def update_regions_available_event(regions_available, event, region_event_idx=None):
    """
    ...
    if region_event_idx is None:
        for i, r in enumerate(regions_available):
            if r['start'] <= event.start < r['start'] + r['len']:
                region_event_idx = i
                break
    ...
    if rel_start + event.length < region['len'] - 1:
        new_regions.append(
            {'start': event.start + event.length + 1, 'len': region['len'] - event.length - rel_start - 1})
...
def N_events_combinations(n, k):
    ...
    for a in range(n):
        if a >= 2 * n - (k + 1):
            n_comb += n_bkp(n, k, a) * n_g(n, a) * c_db(n, a)
    return n_comb
